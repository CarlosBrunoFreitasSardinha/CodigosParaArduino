...
void setup() {
  ...
  pinMode(D1, OUTPUT);
  digitalWrite(D2, 1);
  digitalWrite(D1, 1); //Preparando o GPI, que esta lidago ao RELE
...
void loop() {
  ...
  buf += "<p>RELE <a href=\"?function=RELE0_on\"><button>LIGA</button></a><a href=\"?function=RELE0_off\"><button>DESLIGA</button></a></p>";
  buf += "</html>\n";
  client.print(buf);  //Enviando para o browser a 'pagina' criada.
  client.flush();

  if (req.indexOf("RELE2_on") != -1){digitalWrite(D2, 0);}
  else if (req.indexOf("RELE2_off") != -1){digitalWrite(D2, 1);}
  
  if (req.indexOf("RELE0_on") != -1){digitalWrite(D1, 0);}
  else if (req.indexOf("RELE0_off") != -1){digitalWrite(D1, 1);}
